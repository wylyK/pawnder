      ...
      <div className="flex flex-col">
        <div className="flex bg-white mt-12 py-6 p-8 m-6 mx-64 rounded-3xl">
          ...
          <div>
            ...
            <ProfileTextBox label="Name" type="text" value={nameValue} onChange={handleNameValueChange} disabled={!isEditing}/>
            <ProfileTextBox label="Email" type="email" value={emailValue} onChange={handleEmailValueChange} disabled={!isEditing}/>
            <ProfileTextBox label="Password" type="password" value={passwordValue} onChange={handlePasswordValueChange} disabled={!isEditing}/>
            <ProfileTextBox label="Date of birth" type="date" value={birthValue} onChange={handleBirthValueChange} disabled={!isEditing}/>
            <CountrySelect value={selectedCountry} onChange={handleCountryChange} disabled={!isEditing} />
          </div>    
        </div>
        <div className="ml-64">
          <button className="bg-pink-rose text-white text-xl font-semibold p-2 px-10 rounded-md" onClick={handleEditClick}>
            {isEditing ? 'Save' : 'Edit'}
          </button>
        </div>
      </div>
